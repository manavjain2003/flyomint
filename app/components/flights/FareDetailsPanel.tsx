...
export default function FareDetailsPanel({
    ...
    function lineTotalFor(fare: FareInfo, p: NonNullable<FareInfo["PTCFare"]>[number]) {
        const key = PTC_COUNT_KEY[p.PTC];
...
const discountTotal = grossFareTotal - netFareTotal;
    ...
    return (
        <div className="bg-dark dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 px-4 sm:px-6 py-5">
                ...
                {tabs.map((t) => (
                    <button
                        ...
                        className={`h-8 px-5 rounded-full text-xs font-bold uppercase tracking-wide transition-colors ${
                            tab === t ? "bg-[#FF7626] text-white" : "bg-white dark:bg-gray-900 text-[#FF7626] border border-[#FF7626]"
                        }`}
                    >
                        {t}
                    </button>
            ...
            {tab === "FLIGHT" && (
                <div className="space-y-4">
                    {legs.map((leg, li) => (
                        <div key={li}>
                            {multiLeg && leg.label && (
                                <p className="text-[11px] font-bold uppercase tracking-wide text-[#FF7626] bg-orange-50 dark:bg-orange-950 rounded px-2 py-1 inline-block mb-2">
                                    {leg.label}
                                </p>
                            )}
                            <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 px-5 py-4 divide-y divide-gray-100 dark:divide-gray-800">
                                        ...
                                        {idx < leg.journey.Segments.length - 1 && seg.Layover && (
                                            <div className="flex items-center gap-3 py-3 px-1">
                                                ...
                                                <span className="inline-flex items-center gap-2 bg-[#e8f4fb] text-[#1c8fc7] rounded-lg px-3 py-2 text-xs whitespace-nowrap">
                                                    <HiOutlineClock className="w-4 h-4 shrink-0 text-orange-500 dark:text-orange-400" />
                                                    <span>
            ...
            {tab === "BAGGAGE" && (
                <div className="space-y-3">
                    {legs.every((l) => visibleBaggageFor(l.fare).length === 0) && (
                        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-4 text-sm text-gray-400 dark:text-gray-500 text-center">
                    ...
                    {legs.map((leg, li) => {
                        ...
                        return (
                            <div key={li} className="space-y-2">
                                {multiLeg && leg.label && (
                                    <p className="text-[11px] font-bold uppercase tracking-wide text-[#FF7626] bg-orange-50 dark:bg-orange-950 rounded px-2 py-1 inline-block">
                                        {leg.label}
                                    </p>
                                )}
                                {baggage.map((b, idx) => (
                                    <div
                                        key={`${li}-${b.PTC}-${idx}`}
                                        className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-4 flex items-center gap-4 sm:gap-8 text-sm"
                                    >
                                        ...
                                        <div>
                                            <p className="text-xs text-gray-400 dark:text-gray-500 mb-1">Cabin Baggage</p>
                                            <p className="font-semibold text-gray-900 dark:text-gray-100">{b.CabinBag ?? "—"}</p>
                                        </div>
                                        <div className="w-px h-8 bg-gray-200 dark:bg-gray-700 shrink-0" />
                                        <div>
                                            <p className="text-xs text-gray-400 dark:text-gray-500 mb-1">Check-in Baggage</p>
                                            <p className="font-semibold text-gray-900 dark:text-gray-100">{b.CheckinBaggage ?? "—"}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
            ...
            {tab === "FARE" && (
                <div className="space-y-3">
                            ...
                            <div key={li} className="space-y-2">
                                ...
                                {ptcFares.map((p, idx) => {
                                    ...
                                    return (
                                        <div
                                            key={`${li}-${p.PTC}-${idx}`}
                                            className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-4 text-sm divide-y divide-gray-100 dark:divide-gray-800"
                                        >
                                            <div className="flex items-center justify-between pb-2">
                                                ...
                                                <span className="font-bold text-gray-900 dark:text-gray-100">{formatPrice(lineTotal)}</span>
                                            </div>
                                            <div className="flex items-center justify-between py-2">
                                                <span className="text-gray-500 dark:text-gray-400">Base Fare</span>
                                                <span className="font-semibold text-gray-900 dark:text-gray-100">{formatPrice(p.Fare)}</span>
                                            </div>
                                            <div className="flex items-center justify-between py-2">
                                                <span className="text-gray-500 dark:text-gray-400">Taxes &amp; Fees</span>
                                                <span className="font-semibold text-gray-900 dark:text-gray-100">{formatPrice(p.Tax)}</span>
                                            </div>
                                            {p.FareMessage && (
                                                <div className="flex items-center justify-between py-2">
                                                    <span className="text-gray-500 dark:text-gray-400">Discount</span>
                                                    <span className="font-semibold text-green-600 dark:text-green-400">{formatOffMessage(p.FareMessage)}</span>
                                                </div>
                                            )}
                                        </div>
                                    );
                    ...
                    {legs.map((leg, li) => (
                        <div key={li} className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-4 flex items-center justify-between text-sm">
                            <span className="text-gray-500 dark:text-gray-400">
                                {multiLeg && leg.label ? `${leg.label} Seats Left` : "Seats Left"}
                            </span>
                            <span className="font-semibold text-gray-900 dark:text-gray-100">{leg.fare.Seats}</span>
                        </div>
                    ))}
                </div>
            )}
