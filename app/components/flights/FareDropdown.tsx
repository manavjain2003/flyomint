...
export function FareDropdownPanel({
    ...
    return (
        <div className="border border-gray-100 dark:border-gray-800 rounded-2xl overflow-hidden bg-white dark:bg-gray-900 shadow-sm">
            <div className="grid grid-cols-[1.2fr_0.9fr_0.9fr_minmax(140px,auto)] gap-3 px-5 py-3 bg-gray-50 dark:bg-gray-900 text-[11px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide">
                <span>Fare Type</span>
                <span>Cabin</span>
                <span>Checkin</span>
                <span />
...
{fares.map((f, idx) => {
    ...
    !showBookButton ? "cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800" : ""
} ${isRowSelected ? "bg-[#e8f4fb] dark:bg-[#1c8fc7]/15 shadow-[inset_3px_0_0_0_#1c8fc7]" : ""}`}
                            >
                                <div>
                                    <p className="text-base font-semibold text-gray-800 dark:text-gray-200 leading-snug break-words">
                                        {f.FareType}
                                    </p>
                                   {isRowSelected && (
    <span className="inline-flex items-center gap-1 mt-1.5 px-2 py-0.5 rounded bg-[#1c8fc7] text-white text-[10px] font-bold uppercase tracking-wider">
        <HiCheck className="w-3 h-3" /> Selected
                                ...
                                <p
                                    className={`text-sm leading-snug break-words ${
                                        bag?.CheckinBaggage ? "text-gray-600 dark:text-gray-300" : "text-red-400 dark:text-red-300"
                                    }`}
                                >
        ...
        {discount > 0 && (
            <p className="text-xs font-semibold text-green-600 dark:text-green-400 whitespace-nowrap">
                Extra {formatPrice(discount)} Off
            </p>
        )}
        <p className="text-lg font-bold text-gray-900 dark:text-gray-100 leading-tight whitespace-nowrap">
    ...
    return (
        <div className="w-full sm:w-auto">
            <button
                ...
                className="h-9 px-6 rounded-full text-sm font-semibold whitespace-nowrap transition-colors border bg-white dark:bg-gray-900 text-[#1c8fc7] border-[#1c8fc7] hover:bg-[#e8f4fb] inline-flex items-center gap-1.5"
            >
