...
export default function FilterSidebar({
    ...
    return (
        <div className="bg-white border border-gray-100 rounded-2xl px-4 py-4 sticky top-4">
            ...
            <div className="mb-5">
                ...
                />
                <div className="flex items-center justify-between text-xs font-semibold text-gray-900 mt-1">
                    ...
                    {options.stops.map(({ stops, price }) => (
                        <label key={stops} className="flex items-center justify-between text-sm cursor-pointer">
                            <span className="flex items-center gap-2">
                                ...
                                />
                                <span className="text-gray-700">{stopLabel(stops)}</span>
                            </span>
                            <span className="text-gray-400">{formatPrice(price)}</span>
                        </label>
                    ))}
                    {options.stops.length === 0 && <p className="text-xs text-gray-400">No options yet</p>}
                </div>
            </div>

            {/* Airlines */}
            <div className="mb-5">
                <p className="text-sm font-semibold text-gray-700 mb-2">Airlines</p>
                <div className="space-y-2 max-h-40 overflow-y-auto">
                    {options.airlines.map(({ code, name, price }) => (
                        <label key={code} className="flex items-center justify-between text-sm cursor-pointer">
                            ...
                            <span className="text-gray-400 whitespace-nowrap">{formatPrice(price)}</span>
                        </label>
                    ))}
                    {options.airlines.length === 0 && <p className="text-xs text-gray-400">No options yet</p>}
                </div>
            </div>

            {/* Departure Time */}
            <div>
                <p className="text-sm font-semibold text-gray-700 mb-2">Departure Time</p>
                <div className="space-y-2">
