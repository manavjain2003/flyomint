...
type CheckStep = "payment" | "booking";
...
function BookingConfirmationPage() {
  ...
  const [checkStep, setCheckStep] = useState<CheckStep>("payment");
  ...
  async function handleDownloadTicket() {
    ...
    try {
      const pnr = booking.Journey?.[0]?.AirlinePNR || "";
  ...
  async function handleDownloadInvoice() {
    ...
    try {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `invoice-${refNo || booking.TransactionID}.pdf`;
      document.body.appendChild(a);
      a.click();
      a.remove();
      window.URL.revokeObjectURL(url);
...
function CheckingScreen({ step }: { step: CheckStep }) {
  const steps = [
    ...
    { key: "booking" as const, label: "Confirming your booking", icon: HiOutlinePaperAirplane },
  ];
  const activeIndex = steps.findIndex((s) => s.key === step);

  return (
    <div className="min-h-screen bg-gray-950 flex flex-col items-center justify-center px-4 text-center relative overflow-hidden">
      <div className="pointer-events-none absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-[220px] w-[480px] h-[480px] rounded-full bg-[#1c8fc7]/15 blur-3xl checking-pulse" />
      ...
      <p key={step} className="text-lg font-bold text-white mb-1.5 checking-fade-in">
...
function ActionCard({
  ...
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className="rounded-xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 px-3 py-3 text-left hover:border-[#1c8fc7]/50 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
    >
